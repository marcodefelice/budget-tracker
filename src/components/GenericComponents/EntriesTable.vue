        ...
        <div class="block w-full overflow-x-auto">
            ...
            <div class="container px-4 mx-auto py-3 border border-solid border-blueGray-100 shadow" :class="[
                ...
                <div class="flex flex-wrap">
                    ...
                    <div class="flex-l">
                        <EntryActionDropdown :entryId="entry.id" />
                    </div>
                </div>
...
export default {
    data() {
        return {
            ENTRIES_ROUTE: "/api/entries/account/",
            vue,
            react,
            sketch,
            angular,
    ...
    methods: {
        fixWallet() {
            ...
            axios.post(DOMAIN + "/api/stats/wallet", data, {
                headers: X_API_KEY,
            }).then((resp) => {
        ...
        getEntries(filter, path) {
            ...
            if (path == null) {
                path = DOMAIN + this.ENTRIES_ROUTE + this.selected.account + "/" + _this.filter
            }
        ...
        next(next) {
            this.getEntries('', DOMAIN + this.ENTRIES_ROUTE + this.selected.account + "?page=" + this.pagination.next + this.filter)
            this.pagination.next = next + 1
            this.pagination.previus = next - 1
        },
        previus(previus) {
            this.getEntries('', DOMAIN + this.ENTRIES_ROUTE + this.selected.account + "?page=" + this.pagination.previus + this.filter)
